...
interface UseCommentsReturn {
  ...
  isLoading: boolean;
  error: string | null;
  addComment: (content: string, mentions?: string[], parentCommentId?: string) => Promise<void>;
...
export const useComments = ({
  ...
  const transformFirestoreComment = useCallback((doc: any): Comment => {
    ...
    return {
      ...
      deletedAt: data.deletedAt instanceof Timestamp ? data.deletedAt.toDate() : undefined,
      replies: [],
    };
  }, []);
  ...
  }, [resourceType, resourceId, enableRealtime, transformFirestoreComment, buildCommentTree]);
  ...
  // 댓글에 작성자 정보 추가
  const enrichCommentsWithUserInfo = useCallback(async (comments: Comment[]): Promise<Comment[]> => {
    const enrichComment = async (comment: Comment): Promise<Comment> => {
      const authorInfo = await loadUserInfo(comment.authorId);
      const enrichedReplies = comment.replies ? await Promise.all(
        comment.replies.map(enrichComment)
      ) : [];
  ...
  // 멘션 알림 생성
  const createMentionNotifications = useCallback(async (commentId: string, mentionedUserIds: string[]) => {
    try {
