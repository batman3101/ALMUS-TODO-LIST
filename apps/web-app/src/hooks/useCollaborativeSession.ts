...
import type { WebSocketMessage, UserPresence, EditOperation } from '../services/websocket';
...
export const useCollaborativeSession = ({
  ...
  const handleUserJoined = useCallback((message: WebSocketMessage) => {
    ...
    setSessionState(prev => ({
      ...prev,
      participants: prev.participants.some(p => p.userId === participant.userId)
        ? prev.participants.map(p => p.userId === participant.userId ? participant : p)
  ...
  const handlePresenceUpdated = useCallback((message: WebSocketMessage) => {
    if (message.resourceId !== resourceId) return;
  ...
  const handleUserTyping = useCallback((message: WebSocketMessage) => {
    if (message.resourceId !== resourceId) return;
  ...
  const handleUserStoppedTyping = useCallback((message: WebSocketMessage) => {
    if (message.resourceId !== resourceId) return;
  ...
  // 충돌 감지 로직
  const detectConflicts = (newOperation: EditOperation, existingOperations: EditOperation[]): EditOperation[] => {
    ...
    for (const existing of existingOperations) {
      ...
      if (existing.position.fieldPath === newOperation.position.fieldPath) {
        ...
        const newEnd = newOperation.position.column + (newOperation.length || 0);
        
        if (
          (existing.position.column <= newOperation.position.column && existingEnd > newOperation.position.column) ||
          (newOperation.position.column <= existing.position.column && newEnd > existing.position.column)
        ) {
          conflicts.push(existing);
        }
      }
  ...
  // 타이핑 상태 설정
  const setTyping = useCallback((isTyping: boolean, fieldPath?: string) => {
    if (!sessionState.isActive) return;
  ...
  // 자동 참여
  useEffect(() => {
    if (autoJoin && websocket.isConnected() && !sessionState.isActive && !isConnecting) {
      joinSession();
    }
  ...
  const allParticipants = sessionState.participants.filter(p => p.userId !== user?.id);
  const currentUserPresence = sessionState.participants.find(p => p.userId === user?.id);
  ...
  return {
    ...
    // 유틸리티
    getParticipantById: (userId: string) => sessionState.participants.find(p => p.userId === userId),
    getTypingParticipants: () => sessionState.participants.filter(p => p.isTyping && p.userId !== user?.id),
    isParticipantOnline: (userId: string) => {
      const participant = sessionState.participants.find(p => p.userId === userId);
