...
export const useAuthOptimized = () => {
  ...
  const createAuthUser = useCallback((authUser: any): AuthUser => {
    return {
      ...
      displayName: authUser.user_metadata?.name || authUser.email?.split('@')[0] || '사용자',
  ...
  useEffect(() => {
    ...
    authListener = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : '인증 상태 변경 오류가 발생했습니다.');
      }
    });
  ...
  const signIn = useCallback(async (email: string, password: string) => {
    try {
      ...
      const errorMap: Record<string, string> = {
        ...
        'User not confirmed': '이메일 인증이 필요합니다. 이메일을 확인해주세요.',
        'Email not confirmed': '이메일 인증이 필요합니다. 이메일을 확인해주세요.',
      };

      const errorMessage = errorMap[err.message] || err.message || '로그인에 실패했습니다.';
  ...
  const signUp = useCallback(async (email: string, password: string, name?: string) => {
    try {
      ...
      const errorMap: Record<string, string> = {
        ...
        'Password should be at least': '비밀번호가 너무 약합니다. 6자 이상 입력해주세요.',
        'Invalid email format': '올바른 이메일 형식이 아닙니다.',
        'Signup is disabled': '현재 회원가입이 비활성화되어 있습니다.',
      };

      const errorMessage = errorMap[err.message] || err.message || '회원가입에 실패했습니다.';
      setError(errorMessage);
      throw new Error(errorMessage);
    } finally {
      setLoading(false);
  ...
  const updateUser = useCallback(async (updates: Partial<AuthUser>): Promise<void> => {
    ...
    try {
      ...
      Object.entries(updates).forEach(([key, value]) => {
        if (value !== undefined && ['current_team_id', 'name', 'role', 'is_active', 'avatar'].includes(key)) {
