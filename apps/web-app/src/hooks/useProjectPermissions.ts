...
export const useProjectPermissions = (projectId: string) => {
  ...
  // 프로젝트 권한 부여
  const grantPermission = async (input: CreateProjectPermissionInput): Promise<ProjectPermission> => {
    ...
    try {
      ...
      const permissionRef = doc(collection(db, FIRESTORE_COLLECTIONS.PROJECT_PERMISSIONS));
      ...
      await addDoc(collection(db, FIRESTORE_COLLECTIONS.PROJECT_PERMISSIONS), permissionData);
      ...
      toast.error(error instanceof Error ? error.message : '프로젝트 권한 부여에 실패했습니다.');
      throw error;
    }
  };

  // 프로젝트 권한 업데이트
  const updatePermission = async (input: UpdateProjectPermissionInput): Promise<void> => {
    if (!user) throw new Error('사용자가 로그인되어 있지 않습니다');

    try {
      const permissionRef = doc(db, FIRESTORE_COLLECTIONS.PROJECT_PERMISSIONS, input.id);
  ...
  const revokePermission = async (permissionId: string): Promise<void> => {
    ...
    try {
      ...
      const permissionRef = doc(db, FIRESTORE_COLLECTIONS.PROJECT_PERMISSIONS, permissionId);
  ...
  const getPermissionStats = () => {
    ...
    const roleStats = {
      [ProjectRole.PROJECT_MANAGER]: getPermissionsByRole(ProjectRole.PROJECT_MANAGER).length,
      [ProjectRole.PROJECT_LEAD]: getPermissionsByRole(ProjectRole.PROJECT_LEAD).length,
      [ProjectRole.CONTRIBUTOR]: getPermissionsByRole(ProjectRole.CONTRIBUTOR).length,
