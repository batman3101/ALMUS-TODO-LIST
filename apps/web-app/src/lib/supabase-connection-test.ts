  ...
  try {
    ...
    if (testError) {
      ...
      if (testError.message.includes('infinite recursion')) {
        return {
          isConnected: false,
          error: 'RLS 정책에 순환 참조 문제가 있습니다. 데이터베이스 관리자에게 문의하세요.',
          details: testError
        };
      }
      ...
      if (testError.message.includes('permission denied') || testError.message.includes('RLS')) {
        return {
          isConnected: false,
          error: '데이터베이스 접근 권한이 없습니다. 인증 후 다시 시도해 주세요.',
          details: testError
        };
      }
...
export function validateSupabaseConfig(): {
  ...
  if (!url) {
    ...
    issues.push('VITE_SUPABASE_URL이 기본값입니다. 실제 Supabase 프로젝트 URL로 변경하세요.');
  ...
  if (!key) {
    ...
    issues.push('VITE_SUPABASE_ANON_KEY가 기본값입니다. 실제 Supabase anon key로 변경하세요.');
...
export async function checkDatabaseMigrations(): Promise<{
  ...
  const requiredTables = ['users', 'teams', 'team_members', 'projects', 'tasks'];
