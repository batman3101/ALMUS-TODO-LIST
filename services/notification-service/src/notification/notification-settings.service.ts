...
export class NotificationSettingsService {
  ...
  async createSettings(input: CreateNotificationSettingsInput): Promise<NotificationSettingsType> {
    const settings = this.notificationSettingsRepository.create({
      ...
      taskStatusChange: input.taskStatusChange || NotificationFrequency.IMMEDIATE,
      ...
      systemAnnouncement: input.systemAnnouncement || NotificationFrequency.IMMEDIATE,
      ...
      slackEnabled: input.slackEnabled !== undefined ? input.slackEnabled : false,
      teamsEnabled: input.teamsEnabled !== undefined ? input.teamsEnabled : false,
      kakaoEnabled: input.kakaoEnabled !== undefined ? input.kakaoEnabled : false,
  ...
  async findSettingsByUserId(userId: string): Promise<NotificationSettingsType> {
    return this.notificationSettingsRepository.findOne({
      where: { userId },
    });
  }

  async updateSettings(input: UpdateNotificationSettingsInput): Promise<NotificationSettingsType> {
